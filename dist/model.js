...
export function documentHtml(data, contract = '*', includeQuestions = true) {
  ...
  const answeredQuestionsHtml = n => {
    const answered = n.questions.filter(q => q.visibility !== 'internal' && q.status === 'resolved');
    return answered.length ? `<h3>Beantwortete Fragen</h3><ul class="answered-questions">${answered.map(q => `<li><strong>${e(q.text)}</strong>${q.answer ? `<p class="preserve">${e(q.answer)}</p>` : ''}</li>`).join('')}</ul>` : '';
  };
  ...
  function walk(id, prefix = '') {
    for (const n of children(data, id)) {
      if (included.has(n.id)) {
        ...
        sections.push(`<section class="doc-section" id="doc-${e(n.id)}"><div class="doc-eyebrow">${e(types[n.type])} · ${e(effectiveContract(data, n.id))}</div><h2>${number} ${e(n.title)}</h2>${n.description ? `<p class="preserve">${e(n.description)}</p>` : ''}${n.criteria.length ? `<h3>Abnahmekriterien</h3><ul>${n.criteria.map(c => `<li>${e(c)}</li>`).join('')}</ul>` : ''}${n.exclusions ? `<h3>Nicht im Leistungsumfang</h3><p class="preserve">${e(n.exclusions)}</p>` : ''}${refs.length ? `<h3>Voraussetzungen und Referenzen</h3><ul>${refs.map(r => `<li>${e(r)}</li>`).join('')}</ul>` : ''}${answeredQuestionsHtml(n)}</section>`);
        walk(n.id, number);
